...
#define ACTIONS_LIST_MAX_SIZE 10000
...
static int    g_prompt_use_curl  = 0;
static int    g_prompt_llm_slot = 0;
static char   g_prompt_tmp_path[256] = "";
...
char* LUA_HL_keywords[] = {
    ...
    "true", "until", "while", "require", "print", "type", "pairs", "ipairs",
    "next", "select", "tostring", "tonumber", "error", "assert", "pcall",
    "xpcall", "unpack", "load", "loadfile", "dofile", "rawget", "rawset",
    "setmetatable", "getmetatable", "coroutine", "string", "table", "math",
    "io", "os", "package", "debug",
...
char* RUST_HL_keywords[] = {
    ...
    "println", "print", "panic", "vec", "format", "assert", "assert_eq",
    "assert_ne", "todo", "unimplemented", "unreachable", "dbg",

    "i8|", "i16|", "i32|", "i64|", "i128|", "isize|",
    "u8|", "u16|", "u32|", "u64|", "u128|", "usize|",
    "f32|", "f64|", "bool|", "char|", "str|", "String|",
    "Vec|", "Box|", "Option|", "Result|", "Some|", "None|",
    "Ok|", "Err|", "HashMap|", "HashSet|", NULL
};
...
int editorReadKey() {
    ...
    if (c == '\x1b') {
        ...
        if (seq[0] == '[') {
            if (seq[1] >= '0' && seq[1] <= '9') {
                ...
                if (seq[1] == '[' && seq[2] == '1') {
                    /* F5 = \x1b[15~ */
                    char seq3[3] = {0};
                    if (read(STDIN_FILENO, &seq3[0], 1) == 1 &&
                        read(STDIN_FILENO, &seq3[1], 1) == 1) {
                        if (seq3[0] == '5' && seq3[1] == '~') return KEY_F5;
                    }
                }
                if (seq[2] == '~') {
                    switch (seq[1]) {
        ...
        if (uc >= 0x80) {
            ...
            g_utf8_pending_len = nbytes;
            return MEL_UTF8_CHAR;
        }
        return c;
    }
}
...
int checkFilePermissions(const char* filename) {
    ...
    strncpy(dirname, filename, PATH_MAX - 1);
...
void editorRefreshScreen() {
    ...
    { ssize_t _wr = write(STDOUT_FILENO, ab.buf, ab.len); (void)_wr; }
    abufFree(&ab);
}
...
static int utf8_prev_char_len(const char *chars, int pos) {
    if (pos <= 0) return 0;
    int len = 1;
    while (pos - len > 0 && utf8_is_continuation((unsigned char)chars[pos - len]))
        len++;
    return len;
}
...
int editorRowRenderXToCursorX(editor_row* row, int render_x) {
    ...
    while (cursor_x < row->size) {
        ...
        if (row->chars[cursor_x] == '\t') {
            ...
            clen = utf8_char_len_at(row->chars, cursor_x, row->size);
            if (clen < 1) clen = 1;
            cur_render_x++;
        }
...
void editorUpdateRow(editor_row* row) {
    ...
    for (int j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % MEL_TAB_STOP != 0 && idx < (int)(render_size - 1)) {
                row->render[idx++] = ' ';
            }
        } else if (idx < (int)(render_size - 1)) {
...
static int fileExists(const char* file_name) __attribute__((unused));
...
void editorSave() {
    ...
    if (!fp) {
        ...
        if (!buf) { editorSetStatusMessage("Failed to prepare content"); return; }
        fp = fopen(ec.file_name, "w");
        if (!fp) {
            free(buf);
...
void editorSearchCallback(char* query, int key) {
   ...
   static int saved_cursor_y = -1;   (void)saved_cursor_y;
   static int saved_col_offset = -1; (void)saved_col_offset;
   static int saved_row_offset = -1; (void)saved_row_offset;
...
void editorSearch() {
    int saved_cursor_x = ec.cursor_x; (void)saved_cursor_x;
    int saved_cursor_y = ec.cursor_y;   (void)saved_cursor_y;
    int saved_col_offset = ec.col_offset; (void)saved_col_offset;
    int saved_row_offset = ec.row_offset; (void)saved_row_offset;
...
void revert(Action *action) {
    ...
    switch(action->t) {
        ...
        case DelChar:
            {
                if(action->string) {
                    ec.cursor_x = action->cpos_x - (int)strlen(action->string);
                    ec.cursor_y = action->cpos_y;
                    if (ec.cursor_y < ec.num_rows) {
                        editorRowInsertString(&ec.row[ec.cursor_y], ec.cursor_x, action->string);
                        ec.cursor_x += (int)strlen(action->string);
                    } else {
                        editorInsertRow(ec.num_rows, action->string, strlen(action->string));
                        ec.cursor_x = (int)strlen(action->string);
                    }
...
bool concatWithLastAction(ActionType t, char* str) {
    if(t == InsertChar &&
       ...
       (int)(ec.actions->current->action->cpos_x + strlen(ec.actions->current->action->string)) == ec.cursor_x
        ...
        string = realloc(string, slen + alen + 1);
        memcpy(string + slen, str, alen + 1);
    ...
    for (int y = 0; y < ec.screen_rows; y++) {
        ...
        if (file_row >= ec.num_rows) {
            ...
            for (int j = 0; j < len; j++) {
                ...
                if (iscntrl((unsigned char)c[j]) && (unsigned char)c[j] < 128) {
                    char sym = (c[j] <= 26) ? '@' + c[j] : '?';
                    abufAppend(ab, "\x1b[7m", 4);
                    abufAppend(ab, &sym, 1);
                    abufAppend(ab, "\x1b[m", 3);
    ...
    while (1) {
        ...
        if (c == DEL_KEY || c == CTRL_KEY('h') || c == BACKSPACE) {
            if (buf_len > 0) {
                /* UTF-8 backspace */
                int back = 1;
                while (back < (int)buf_len &&
                       utf8_is_continuation((unsigned char)buf[buf_len - back]))
                    back++;
                buf_len -= back;
                buf[buf_len] = '\0';
            }
            ...
            FILE *xfp = popen("xclip -selection clipboard -o 2>/dev/null || xsel --clipboard --output 2>/dev/null || pbpaste 2>/dev/null", "r");
            if (xfp) {
                char clip[8192]; size_t clen = 0; int ch;
                while ((ch = fgetc(xfp)) != EOF && clen < sizeof(clip)-1) {
                    if (ch == '\n' || ch == '\r') continue;
                    clip[clen++] = (char)ch;
                }
                clip[clen] = '\0';
                pclose(xfp);
                if (clen > 0) {
                    while (buf_len + clen + 1 >= buf_size) { buf_size *= 2; buf = realloc(buf, buf_size); }
                    memcpy(buf + buf_len, clip, clen);
                    buf_len += clen;
                    buf[buf_len] = '\0';
                }
            }
        ...
        } else if (c == MEL_UTF8_CHAR) {
            while (buf_len + g_utf8_pending_len + 1 >= buf_size) { buf_size *= 2; buf = realloc(buf, buf_size); }
            memcpy(buf + buf_len, g_utf8_pending, g_utf8_pending_len);
            buf_len += g_utf8_pending_len;
            buf[buf_len] = '\0';
        } else if (!iscntrl(c) && c < 128) {
            if (buf_len + 1 >= buf_size) { buf_size *= 2; buf = realloc(buf, buf_size); }
            buf[buf_len++] = (char)c;
            buf[buf_len] = '\0';
        }
...
void editorInsertOllamaResponse(void);
void editorInsertOllamaResponseCurl(void);
static void editorSendPromptAndRestore(void);
static void editorCancelPromptMode(void);
    ...
    switch (c) {
        ...
        case CTRL_KEY('t'):
            /* Ctrl-T — вставка из системного буфера обмена (xclip/xsel/pbpaste) */
            {
                ...
                if (xfp) {
                    char clip[65536]; size_t clen = 0; int ch;
                    while ((ch = fgetc(xfp)) != EOF && clen < sizeof(clip)-1)
                        clip[clen++] = (char)ch;
                    clip[clen] = '\0';
                    pclose(xfp);
                    if (clen > 0) {
                        ...
                        for (ln = strtok_r(clip, "\n", &sp); ln;
                             ln = strtok_r(NULL, "\n", &sp)) {
                            if (!first) makeAction(NewLine, NULL);
                            if (strlen(ln) > 0)
                                makeAction(InsertChar, strndup(ln, strlen(ln)));
                            first = 0;
                        }
                        editorSetStatusMessage("Clipboard pasted (%zu chars)", clen);
                    }
                } else {
                    editorSetStatusMessage("xclip/xsel not found");
                }
            }
            break;
        case CTRL_KEY('p'):
		...
		case CTRL_KEY('w'):
            ...
            break;
        case CTRL_KEY('k'):
            editorInsertOllamaResponseCurl();
            break;
        case ARROW_UP:
        case ARROW_DOWN:
        case ARROW_LEFT:
        ...
        case CTRL_KEY('l'):
        case CTRL_KEY('o'):
            if (g_prompt_mode) editorSendPromptAndRestore();
            break;
        case '\x1b': // Escape key
            if (g_prompt_mode) editorCancelPromptMode();
            break;
        case CTRL_KEY('z'):
            undo();
            break;
...
void editorDisplayHelpPage() {
    ...
    printf("MEL - Mini Editor for Linux v.1.3.1\r\n\r\n");
	...
	printf("Ctrl-Left     Move cursor one word left\r\n");
	printf("Ctrl-Right    Move cursor one word right\r\n");
    ...
    printf("-----------------------------------------\r\n");
    printf("Supports highlighting for C,C++,Java,Bash,Mshell,Python,PHP,Javascript,JSON,XML,SQL,Ruby,Go,Lua,Rust.\r\n");
	printf("License: Public domain libre software GPL3,v.1.3.1, 2025\r\n");
...
static void editorFreeRowsOnly(void) {
    if (!ec.row) return;
    for (int i = 0; i < ec.num_rows; i++) {
        if (ec.row[i].chars)     free(ec.row[i].chars);
        if (ec.row[i].render)    free(ec.row[i].render);
        if (ec.row[i].highlight) free(ec.row[i].highlight);
    }
    free(ec.row);
    ec.row      = NULL;
    ec.num_rows = 0;
...
static void editorEnterPromptMode(int slot) {
    memset(g_utf8_pending, 0, sizeof(g_utf8_pending));
    ...
    snprintf(g_prompt_tmp_path, sizeof(g_prompt_tmp_path),
             "/tmp/mel_prompt_%d.txt", (int)getpid());
    FILE *fp = fopen(g_prompt_tmp_path, "w");
    if (fp) fclose(fp);
    ec.file_name = strdup(g_prompt_tmp_path);

    if (g_prompt_use_curl) {
        editorSetStatusMessage(
            "LLM Prompt (curl/local) | write prompt | Ctrl-O=send  ESC=cancel  Ctrl-T=paste");
    } else {
        editorSetStatusMessage(
            "LLM Prompt (ollama%d) | write prompt | Ctrl-O=send  ESC=cancel  Ctrl-T=paste",
            slot);
    }
}

static void editorSendPromptAndRestore(void) {
    if (!g_prompt_mode) return;
    ...
    if (!content || len == 0) {
        ...
        editorSetStatusMessage("Empty prompt");
        return;
    }
    ...
    if (g_prompt_use_curl) {
        g_prompt_use_curl = 0;
        remove(g_prompt_tmp_path);
        editorSendCurlResponse(content);
        return;
    }
    ...
    char ms_path[256];
    snprintf(ms_path, sizeof(ms_path), "/tmp/mel_llm_%d.ms", (int)getpid());
    FILE *ms = fopen(ms_path, "w");
    if (!ms) { free(sq); remove(g_prompt_tmp_path); return; }
    fprintf(ms, "#!/bin/mshell\nollama%d '%s'\necho ''\necho MEL_RESPONSE_END\n",
            g_prompt_llm_slot, sq);
    fclose(ms);
    free(sq);
    chmod(ms_path, 0755);
    remove(g_prompt_tmp_path);

    editorSetStatusMessage("Calling ollama%d... wait", g_prompt_llm_slot);
    editorRefreshScreen();

    FILE *fp = popen(ms_path, "r");
    if (!fp) { remove(ms_path); editorSetStatusMessage("popen failed"); return; }

    size_t resp_size=262144, resp_len=0;
    char *resp_buf = malloc(resp_size);
    char read_buf[65536];
    while (fgets(read_buf, sizeof(read_buf), fp)) {
        if (strstr(read_buf, "MEL_RESPONSE_END")) break;
        if (strncmp(read_buf,"[mide]",6)==0) continue;
        if (strncmp(read_buf,"  rx:",5)==0) continue;
        if (strncmp(read_buf,"  tx:",5)==0) continue;
        size_t chunk=strlen(read_buf);
        if (resp_len+chunk+1>=resp_size) {
            resp_size=(resp_len+chunk+1)*2;
            char *tmp=realloc(resp_buf,resp_size);
            if (!tmp) break;
            resp_buf=tmp;
        }
        memcpy(resp_buf+resp_len, read_buf, chunk);
        resp_len+=chunk;
    }
    resp_buf[resp_len]=0;
    pclose(fp);
    ...
    if (resp_len==0) {
        free(resp_buf);
        editorSetStatusMessage("No response from ollama%d", g_prompt_llm_slot);
        return;
    }

    char *ln, *sp;
    int first=1;
    for (ln=strtok_r(resp_buf,"\n",&sp); ln; ln=strtok_r(NULL,"\n",&sp)) {
        if (!first) makeAction(NewLine, NULL);
        if (strlen(ln)>0) makeAction(InsertChar, strndup(ln,strlen(ln)));
        first=0;
    }
    free(resp_buf);
    editorSetStatusMessage("ollama%d response inserted (%zu chars)", g_prompt_llm_slot, resp_len);
}
...
static void editorCancelPromptMode(void) {
    ...
    editorSetStatusMessage("Prompt cancelled");
}


void editorInsertOllamaResponse() {
    char* slot_str = editorPrompt("Ollama model (1/2/3): %s", NULL);
    if (!slot_str) return;
    int slot = atoi(slot_str);
    free(slot_str);
    if (slot < 1 || slot > 3) {
        editorSetStatusMessage("Invalid model: enter 1, 2 or 3");
        return;
    }
    editorEnterPromptMode(slot);
}
...
int readOllamaConfig(const char *config_path) {
    ...
    if (!file) { editorSetStatusMessage("Could not open config file"); return 0; }
    char line[256];
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strncmp(line, "OLLAMA1_API_URL=", 16) == 0)
            ...
            ollama_config.model[sizeof(ollama_config.model) - 1] = '\0';
        }
    }
    fclose(file);
...
char* callOllamaAPI(const char* prompt) {
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            ...
            json_object *parsed = json_tokener_parse(response.buf);
            json_object *resp_obj;
            if (json_object_object_get_ex(parsed, "response", &resp_obj))
                output = strdup(json_object_get_string(resp_obj));
...
static void editorSendCurlResponse(char *content) {
    ...
    if (response) {
        ...
        free(response);
        editorSetStatusMessage("Ollama (curl) response inserted");
    } else {
        editorSetStatusMessage("No response from Ollama (curl)");
    }
}
...
int main(int argc, char* argv[]) {
    ...
    enableRawMode();
	editorSetStatusMessage(" Ctrl-Q to quit | Ctrl-S to save | Ctrl-Left/Right to move by words | (mel -h | --help for more info)");
